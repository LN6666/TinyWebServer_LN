...
#ifndef MAIN_C_LST_TIMER_H
#define MAIN_C_LST_TIMER_H
#include <time.h>
#include "log.h"
//连接资源结构体成员需要用到定时器类
class util_timer;//前向声明
struct client_data//用户数据结构，连接资源
{
...
{
public:
    util_timer() : prev(NULL), next(NULL) {}//构造函数，参数列表初始化

public:
    time_t expire;//任务超时时间，这里使用绝对时间
    void (*cb_func)(client_data *);//任务回调函数，回调函数处理的客户数据，由定时器的执行者传递给回调函数
    //函数指针，强调是指针，该指针指向的函数具有client_data*类型参数，并且返回值是void类型的
    client_data *user_data;//链表节点的数据部分
...
public:
    ...
    void add_timer(util_timer *timer)
    {
        ...
        if (!head)//如果头节点为空，也就是链表为空，该节点设置为链表的头节点和尾节点
        {
            head = tail = timer;
            return;
        }
    ...
    void adjust_timer(util_timer *timer)
    {
        ...
        else
        {
            ...
            timer->next->prev = timer->prev;//这里反转过来互换顺序也是可以的
    ...
    void tick()
    {
        ...
        while (tmp)
        {
            ...
            if (head)
            {
                head->prev = NULL;//如果head不为空，就执行这个
            }
            delete tmp;
            tmp = head;
        }
